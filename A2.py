...
def submit_value():
    ...
    if book_title_info == "" or user_info == "":
        ...
        user_result = es_conn.search(index="rratings", query=elastic_query_user)# The value of the index parameter has to be normally users(index) but we put it rratings for the continue of the project
        ...
        if len(user_result['hits']['hits']) == 0:
            ...
            for i in range(len(data_set['isbn'])):#Can we make it a function?
                ...
                if math.isnan(Average[-1]):# Because we know that the user_rating can be nan we put -1 to Average, because we want to check only the last element of the list. If it is nan we pop it.
                    Average.pop()
